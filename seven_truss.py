...
class Truss_Analysis(Group):
    ...
    def setup(self):
        ...
        indeps.add_output("P1", -(3 ** .5) * F / 2)
        indeps.add_output("P2", -F)
        indeps.add_output("P3", F)
        indeps.add_output("P4", ((3 ** .5) + 2) * F / 2)
        indeps.add_output("P5", F)
        indeps.add_output("P6", -F * (2 ** .5))
        indeps.add_output("P7", F / 2)
        ...
        trussDE = self.add_subsystem("trussDE", truss())
        trussEA = self.add_subsystem("trussEA", truss())
        trussAD = self.add_subsystem("trussAD", truss())
        trussBD = self.add_subsystem("trussBD", truss())

        self.connect("indeps.P1", ["trussAB.P"])
        self.connect("indeps.P2", ["trussBC.P"])
        self.connect("indeps.P3", ["trussCD.P"])
        self.connect("indeps.P4", ["trussDE.P"])
        self.connect("indeps.P5", ["trussEA.P"])
        self.connect("indeps.P6", ["trussAD.P"])
        self.connect("indeps.P7", ["trussBD.P"])
        ...
        self.connect("trussDE.sigma", ["con4.sigma"])
        self.connect("trussEA.sigma", ["con5.sigma"])
        self.connect("trussAD.sigma", ["con6.sigma"])
        self.connect("trussBD.sigma", ["con7.sigma"])
        ...
        self.connect("indeps.A4", ["trussDE.A", "obj_cmp.A4"])
        self.connect("indeps.A5", ["trussEA.A", "obj_cmp.A5"])
        self.connect("indeps.A6", ["trussAD.A", "obj_cmp.A6"])
        self.connect("indeps.A7", ["trussBD.A", "obj_cmp.A7"])
...
print("P6 = ", prob["indeps.P6"])
print("A7 = ", prob["indeps.A7"])
print("P7 = ", prob["indeps.P7"])
